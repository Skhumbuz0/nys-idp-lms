...
def update_participant_progress(db, participant_id: str):
    ...
    progress += len({q.week for q in quizzes}) * 15.0 # 60% max
    ...
    progress += (action_pct / 100.0) * 35.0 # 35% max
    
    if p.final_idp_complete: progress += 10.0 # Wait, 5+60+35+10 = 110. Adjusted: Reg 5, Quiz 45, Action 40, Final 10 = 100
    progress = min(100.0, 5.0 + (len({q.week for q in quizzes}) * 11.25) + ((action_pct / 100.0) * 40.0) + (10.0 if p.final_idp_complete else 0.0))
...
def process_register(request: Request, db = Depends(get_db), full_name: str = Form(...), email: str = Form(...), phone: str = Form(""), age_range: str = Form(...), main_goal: str = Form(...)):
    ...
    return templates.TemplateResponse("register_success.html", {"request": request, "pid": pid})

@app.get("/daily-action", response_class=HTMLResponse)
def daily_action_hub(request: Request, db = Depends(get_db)):
    # Simple hub to pick a day, or redirect to first incomplete day
    return templates.TemplateResponse("daily_action_hub.html", {"request": request, "curriculum": CURRICULUM})
...
def daily_action_form(request: Request, day: int):
    ...
    return templates.TemplateResponse("daily_action.html", {"request": request, "day": day, "day_data": CURRICULUM[day]})
...
def dashboard(request: Request, db = Depends(get_db)):
    ...
    return templates.TemplateResponse("dashboard.html", {
        "request": request, "total": total, "completed": completed,
        "risk_dict": risk_dict, "participants": participants
    })
